...
-- Base cadastral enriquecida com informações do histórico
base AS (
    SELECT
        a.ID_CLIENTE,
        a.SAFRA_REF,
        ...
        c.DATA_CADASTRO,
        -- Conversão do CEP_2_DIG para inteiro, tratando valores 'na'
        CAST(
            CASE
...
-- Cálculo de idade da base, estado e região a partir do CEP_2_DIG
fs_cadastral_estado AS (
    SELECT
        *,
        DATEDIFF('{dt_ref}', DATA_CADASTRO) AS IDADE_BASE,
        -- Mapeamento do CEP_2_DIG para ESTADO
        CASE
            ...
            ELSE NULL
        END AS ESTADO,
        -- Mapeamento do ESTADO para REGIAO
        CASE
            WHEN (CASE
                    WHEN CEP_2_DIG BETWEEN 1 AND 19 THEN 'SP'
                    WHEN CEP_2_DIG BETWEEN 20 AND 28 THEN 'RJ'
...
-- Seleção final das colunas relevantes
SELECT 
    ID_CLIENTE,
    ID_DOCUMENTO,
    DDD,
